...
function DeploymentServer(options)
{
	...
	function listen(request, response)
	{
		...
		request.on('end', function()
		{
			...
			if (manual)
			{
				var pageLog = new WebPageLog(filteredLog, response);
			...
			else
			{
				...
				if (options.emailUser)
				{
					...
					{
						emailLog.sendEmail(error, result);
					});
					return;
				}
 ...
 */
var WebPageLog = function(log, response)
{
	// self-reference
	var self = this;
...
function testWebPageLog(callback)
{
	...
	var webPage = new WebPageLog(log, response);
...
var EmailLog = function(options, log)
{
	...
	// attributes
	var response = new WebPageResponse();
	var pageLog = new WebPageLog(log, response);
	 ...
	 */
	self.sendEmail = function(error, result)
	{
		pageLog.close();
		if (!response.finished)
		{
			log.error('Response not yet finished; not sending email');
			return;
		}
		var attachments = [ {
