...
var WebPageLog = function(response)
{
	...
	response.write('<html>\n<head><meta charset="UTF-8"><title>Deployment running</title></head>\n<body>\n');
	response.write('<h1>Started Deployment</h1>\n');

	for (var level in levels)
	{
		self[level] = getShow(levels[level]);
	}

	/**
	 * Get a function to show a message in color.
	 */
	function getShow(color)
	{
		return function(message)
		{
			for (var original in replacements)
			{
				message = message.replaceAll(original, replacements[original]);
			}
			response.write('<p style="color: green">\n' + message + '\n</p>\n');
		}
	}
