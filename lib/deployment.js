...
var Deployment = function(options, log)
{
	...
	self.run = function(callback)
	{
		...
		if (options.testDirectory)
		{
			...
			tasks.push(getExecerIn('npm test', options.testDirectory));
		}
		...
		if (options.deploymentCommand)
		{
			...
			tasks.push(getExecerIn(options.deploymentCommand, directory));
		}
	...
	function update(directory, callback)
	{
		...
		async.waterfall([
			getExecerIn('git pull', directory),
			getExecerIn('npm install', directory),
		], callback);
	}
	 ...
	 */
	function getExecerIn(command, directory)
	{
